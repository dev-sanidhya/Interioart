...
## Second redesign + AI CRM detail (2026-07-23, later still)
User rejected the terracotta/Newsreader revert too ("looks a lot fucked") and pointed at the anthropic-skills `frontend-design` skill on GitHub. Fetched `SKILL.md` directly (not installed locally, pulled via raw.githubusercontent.com) — its calibration section explicitly names "warm cream + high-contrast serif + terracotta accent" as the single most common AI-template look right now. That's exactly the combination used in every iteration so far, which explains the recurring complaint regardless of which specific hex/font was swapped in.

New approach, grounded in the skill's actual guidance (ground the design in the subject's own world, not a generic "premium editorial" default): InterioArty's real signature is spatial precision — VR walkthroughs, floor plans — so the system now borrows from architectural drafting instead of magazine editorial:
- Palette: vellum paper (`#ece6d8`) + ink + a cool "blueprint" blue accent (`#3b5a6b`) + a sparing warm "brass" accent (`#9c7a4a`) — first accent color used all session that isn't a warm red/brown/green in the "premium" family.
- Type: Piazzolla (serif, display only) + IBM Plex Sans (body) + IBM Plex Mono (labels, nav, captions, annotations) — the mono-for-technical-labels move is the actual signature, ties directly to "spec sheet / drafting" concept.
- Signature device: corner registration marks (`.marks`) on photography and dimension-style annotations (`.dim` — tick line + mono label) used for real data (specs, process steps, image dimensions) instead of decorative numbered markers — the skill explicitly flags fake 01/02/03 sequences on non-sequential content as templated, so service listing lost its numbering in favor of a "Type — Residential" spec tag.
- New "Statement" section directly answers the "bigger images, bigger text, abstract" ask: a large image bleeds from the right edge of the section while an oversized serif headline overlaps it on the left, protected by a soft vellum text-shadow halo for legibility; supporting copy is pulled back into the safe (non-overlapping) zone.
- Footer rebuilt as an architectural drawing title-block (labeled cells: Studio / Contact / Location), reinforcing the concept instead of being a generic link list.
